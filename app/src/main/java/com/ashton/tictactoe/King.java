...
public class King extends piece {
	...
	public King (char name, char color) {
		super(name, color);
		inCheck = false;
		beenMoved = false;
	}
			...
			if(this.color == 'w') {
				...
				if (endY == 0 && endX == 2) {
					...
					if ( (board[0][0].name == 'r') && !board[0][0].beenMoved) {
						if (checkEmpty(startX, 1, endY, board)) {
							board[endY][endX] = board[startY][startX];
							board[startY][startX] = null;
							board[0][3] = board[0][0];
							board[0][0] = null;
				...
				if (endY == 0 && endX == 5) {
					if ( (board[0][7].name == 'r') && !board[0][7].beenMoved) {
						if (checkEmpty(startX, 6, endY, board)) {
							board[endY][endX] = board[startY][startX];
							board[startY][startX] = null;
							board[0][4] = board[0][7];
							board[0][7] = null;
				...
				if (endY == 7 && endX == 5) {
					if ( (board[7][7].name == 'r') && !board[7][7].beenMoved) {
						if (checkEmpty(startX, 7, endY, board)) {
							board[endY][endX] = board[startY][startX];
							board[startY][startX] = null;
							board[7][5] = board[7][7];
							board[7][7] = null;
	...
	public boolean checkEmpty(int startX, int endX, int y, piece board[][]) {
		...
		if (checkValidCordinates(endX, 0, startX, 0)) {
